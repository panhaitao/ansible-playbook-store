...
timeout 200 $(whereis -b kubectl | awk '{print $2}') exec -n alauda-system $(grep "lightkeeper-[a-z0-9]\{3,\}-[a-z0-9]\{3,\}$" /tmp/pod.list 2>/dev/null | head -n 1) sh /migrate.sh  && sleep 10 >>/alauda/alauda.log 2>&1
if [[ $? -ne 0 ]]; then echo -e "\033[41;37m lightkeeper migrate failed \033[0m" ;else echo "lightkeeper migrate successed" ;fi

timeout 200 $(whereis -b kubectl | awk '{print $2}') exec -n alauda-system $(grep "lightkeeper-[a-z0-9]\{3,\}-[a-z0-9]\{3,\}$" /tmp/pod.list 2>/dev/null | head -n 1) sh  /lightkeeper/scripts/create_weblab_json.sh >>/alauda/alauda.log 2>&1
if [[ $? -ne 0 ]]; then echo -e "\033[41;37m lightkeeper migrate failed \033[0m" ;else echo "lightkeeper migrate successed" ;fi
...
timeout 20 $(whereis -b kubectl | awk '{print $2}') exec -n alauda-system $(grep "morgans-[a-z0-9]\{3,\}-[a-z0-9]\{3,\}$" /tmp/pod.list 2>/dev/null | head -n 1) python /morgans/migration/api/migrate.py  >>/alauda/alauda.log 2>&1
if [[ $? -ne 0 ]]; then echo -e "\033[41;37m morgans migrate failed \033[0m" ;else echo "morgans migrate successed" ;fi

timeout 20 $(whereis -b kubectl | awk '{print $2}') exec -n alauda-system $(grep "morgans-[a-z0-9]\{3,\}-[a-z0-9]\{3,\}$" /tmp/pod.list 2>/dev/null | head -n 1) sh /morgans/migration/db/migrate.sh  >>/alauda/alauda.log 2>&1
if [[ $? -ne 0 ]]; then echo -e "\033[41;37m morgans migrate failed \033[0m" ;else echo "morgans migrate successed" ;fi
for i in `kubectl get ns |awk '{print $1}' |sed 1d`; do  kubectl label ns $i project.alauda.io/name=system --overwrite; done
